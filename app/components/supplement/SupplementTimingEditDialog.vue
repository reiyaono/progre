...
// 摂取タイミングの追加/編集（名前のみのマスタ）。MethodEditDialog 踏襲。
import type { SupplementTiming } from '~/types/db'

const props = defineProps<{ model?: SupplementTiming | null }>()
...
<template>
  <UiModalDialog :title="isEdit ? 'タイミングを編集' : 'タイミングを追加'" @close="emit('close')">
    <form @submit.prevent="onSubmit">
      <label class="field">
        タイミング名
        <input v-model="name" type="text" placeholder="例: 筋トレ後" required />
      </label>
